...
#Grabs gravity from the project settings and syncs them with RidigBody nodes
...
@export var character_turn_decay: float = 20.0
...
func look_toward_direction(direction: Vector3, delta: float) -> void:
	...
	rig_pivot.global_transform = rig_pivot.global_transform.interpolate_with(
		target_transform,
		character_turn_decay - exp(-character_turn_decay * delta)
	)
